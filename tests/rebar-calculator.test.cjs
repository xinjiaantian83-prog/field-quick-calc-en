...
test("ASTM and metric tables contain the requested sizes and standard unit weights", () => {
...
test("straight wall computes count, yield, stock bars, remaining length, and #4 weight", () => {
  ...
  near(calc.mmToLength(result.remainingLengthMm, "ft"), 59);
  near(result.estimatedWeightLb, 21 * (7 + 8 / 12) * 0.668, 1e-8);
});

test("L-shaped metric wall applies allowance and calculates exact partial-bar remainder", () => {
  const result = calc.calculate({
    system: "metric", mode: "lshape", sizeId: "m12",
    wallLength: 10, verticalLeg: 1.5, horizontalLeg: 0.8,
    cover: 50, spacing: 200, bendMethod: "allowance", bendAdjustment: 40,
    stockLength: 12
  });
  assert.equal(result.valid, true);
  assert.equal(result.pieceCount, 51);
  near(calc.mmToLength(result.cutLengthMm, "m"), 2.24);
  assert.equal(result.piecesPerBar, 5);
  assert.equal(result.barsRequired, 11);
  near(calc.mmToLength(result.remainingLengthMm, "m"), 17.76);
  near(result.estimatedWeightKg, 51 * 2.24 * 0.888, 1e-8);
});

test("bend deduction, custom stock lengths, and all supported size weights stay consistent", () => {
  const deducted = calc.calculate({
    system: "imperial", mode: "lshape", sizeId: "us3",
    wallLength: 12, verticalLeg: 5, horizontalLeg: 3,
    cover: 2, spacing: 12, bendMethod: "deduction", bendAdjustment: 2,
    stockLength: 30
  });
  near(calc.mmToLength(deducted.cutLengthMm, "ft"), 7.5);
  assert.equal(deducted.piecesPerBar, 4);
  assert.equal(deducted.barsRequired, 4);

  for (const size of data.imperial.sizes) {
    const result = calc.calculate({
      system: "imperial", mode: "straight", sizeId: size.id,
      wallLength: 1, wallHeight: 1, cover: 0, spacing: 12, stockLength: 20
    });
    near(result.estimatedWeightLb, 2 * size.weightLbFt, 1e-8);
  }
  for (const size of data.metric.sizes) {
    const result = calc.calculate({
      system: "metric", mode: "straight", sizeId: size.id,
      wallLength: 1, wallHeight: 1, cover: 0, spacing: 1000, stockLength: 6
    });
    near(result.estimatedWeightKg, 2 * size.weightKgM, 1e-8);
  }
});

test("invalid and zero values return safe errors instead of impossible stock counts", () => {
...
test("Imperial and metric equivalents preserve count and stock decisions", () => {
  const imperial = calc.calculate({
    system: "imperial", mode: "straight", sizeId: "us4",
    wallLength: 20, wallHeight: 8, cover: 2, spacing: 12, stockLength: 20
  });
  const metric = calc.calculate({
    system: "metric", mode: "straight", sizeId: "m12",
    wallLength: 6.096, wallHeight: 2.4384, cover: 50.8, spacing: 304.8, stockLength: 6.096
  });
  ...
  near(metric.remainingLengthMm, imperial.remainingLengthMm, 1e-5);
});
